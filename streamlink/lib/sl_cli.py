...
from lib.folder_handler import file_move
...
def get_video(argments):
    ...
    try:
        ...
        if stream_fd:
            try:
                print(f'close {stream_name}')
                log.info(f'close {stream_name}')
                stream_fd.close()
            except KeyboardInterrupt:
                error_code = 130
                # Close output
                ...
                try:
                    file_move(file_name, target_file_name)
                    print(f'{target_file_name} file moved')
                except Exception as e:
                    print(f'{stream_name} {e} occured during moving video from tmp to video')
                print(f'successfully closed {stream_name}')
